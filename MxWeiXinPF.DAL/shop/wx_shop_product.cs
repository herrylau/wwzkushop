...
namespace MxWeiXinPF.DAL
{
	...
	public partial class wx_shop_product
	{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
            ...
            strSql.Append(@" id,wid,categoryId,brandId,sku,productName,shortDesc,unit,weight,description,seo_title,seo_keywords,
                            seo_description,focusImgUrl,thumbnailsUrll,recommended,latest,hotsale,specialOffer,costPrice,marketPrice,
                            salePrice,upselling,stock,addDate,vistiCounts,sort_id,productionDate,ExpiryEndDate,updateDate,catalogId ");
			strSql.Append(" FROM wx_shop_product ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		/// 得到一个对象实体
		/// </summary>
		public MxWeiXinPF.Model.wx_shop_product DataRowToMasterModel(DataRow row)
        {
            MxWeiXinPF.Model.wx_shop_product model = new MxWeiXinPF.Model.wx_shop_product();
            if (row != null)
            {
                ...
                if (row["firstImageUrl"] != null && row["firstImageUrl"].ToString() != "")
                {
                    model.firstImageUrl = row["firstImageUrl"].ToString();
                }
                if (row["categoryName"] != null && row["categoryName"].ToString() != "")
                {
        ...
        public DataSet GetProductFrontPageList(int top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(@" (select top 1 original_path from dbo.wx_shop_albums where  
                            productId=p.id ) as firstImageUrl,p.id,p.wid,p.categoryId,p.brandId,p.sku,p.productName,p.shortDesc,p.unit,p.weight,p.description,p.seo_title,
                            p.seo_keywords,p.seo_description,p.focusImgUrl,p.thumbnailsUrll,p.recommended,p.latest,p.hotsale,
                            p.specialOffer,p.costPrice,p.marketPrice,
                            p.salePrice,p.upselling,p.stock,p.addDate,p.vistiCounts,p.sort_id,p.productionDate,p.ExpiryEndDate,p.updateDate,
                            p.catalogId ,c.title as categoryName ");
            strSql.Append(" FROM wx_shop_product p join wx_shop_category c on p.catalogId=c.id ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public IList<MxWeiXinPF.Model.wx_shop_product> GetProductList(int Top, string strWhere, string filedOrder) {
            var ds = GetProductFrontPageList(Top,strWhere,filedOrder);
            if (ds!=null && ds.Tables[0]!=null && ds.Tables[0].Rows.Count>0)
            {
                IList<MxWeiXinPF.Model.wx_shop_product> result = new List<MxWeiXinPF.Model.wx_shop_product>();
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    result.Add(DataRowToMasterModel(dr));
                }
                return result;
            }
            return null;
        ...
        public int Add(Model.wx_shop_product model)
        {
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@brandId", SqlDbType.Int,8),
					...
					new SqlParameter("@stock", SqlDbType.Int,8),
					new SqlParameter("@addDate", SqlDbType.DateTime),
					new SqlParameter("@vistiCounts", SqlDbType.Int,8),
                    ...
                    new SqlParameter("@catalogId", SqlDbType.Int,8),
            ...
            if (model.attrs != null)
            {
                ...
                foreach (Model.wx_shop_productAttr_value modelt in model.attrs)
                {
                    ...
                    SqlParameter[] parameters3= {
					    new SqlParameter("@productId", SqlDbType.Int,8),
					    new SqlParameter("@attributeId", SqlDbType.Int,8),
        ...
        public bool Delete(int id)
        {
            ...
            //SqlParameter[] parameters4 = {
            //        new SqlParameter("@article_id", SqlDbType.Int,8)};
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@" select  link_url='',(select top 1 original_path from dbo.wx_shop_albums where  
                            productId=p.id) as productpic,* from wx_shop_product p ");
